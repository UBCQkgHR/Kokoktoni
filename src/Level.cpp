    ...
    for (auto i = 0; i < 2; ++i) {
        if (levelData.contains("layers") && levelData["layers"].is_array() &&
            ...
            if (layer.contains("data") && layer["data"].is_array()) {
                ...
                std::cout << "Поле 'data' не найдено или не массив"
                          << std::endl;
            }
        } else {
        ...
        x_size =
            levelData.contains("width") ? levelData["width"].get<int>() : 0;
        ...
        for (int y = 0; y < y_size; ++y) {
            for (int x = 0; x < x_size; ++x) {
                ...
                if (layer_class == "CollisionLayer") {
                    ...
                    /* sf::RectangleShape
          block(sf::Vector2f(31.f, 31.f)); block.setPosition(32.f *
          x, 32.f * y);
                     // block.setTexture(&blockTexture);
                     block.setTextureRect(
                         sf::IntRect(32 * (maptit % 16), 32 * (maptit /
          16), 32, 32));
                     // block.setFillColor(sf::Color(100, 100, 100));
