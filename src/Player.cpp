...
void Player::checkCollision(const Level &level) {
    ...
    int topTile = (int)(bounds.top / TILE);
    sf::Vector2<float> pos = bounds.getPosition();

    printf("%f\n", pos.x);
    int bottomTile = (int)((bounds.top + bounds.height - 1) / TILE);
    if (velocity.x > 0) {
        int rightTile = (int)((bounds.left + bounds.width) / TILE);
