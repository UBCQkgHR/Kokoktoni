  ...
  /* void checkCollisionWithEnemies(T &player,
     ...
     for (auto &enemy : enemies) {
       if (player.getBounds().intersects(enemy->getBounds())) {
         if (player.getVelocity().y > 0 && player.getBounds().top +
           ...
           if (player.getBounds().left < enemy->getBounds().left) {
             player.setPosition(enemy->getBounds().left -
 ...
 */
  //************************************************************
  void checkCollisionWithEnemies(T &player, Enemy &enemy, float deltaTime) {
    if (player.getBounds().intersects(enemy.getBounds())) {
      if (player.getVelocity().y > 0 && player.getBounds().top +
                                                player.getBounds().height -
                                                enemy.getBounds().top <
        ...
        player.takeDamage(1);
        if (player.getBounds().left < enemy.getBounds().left) {
          player.setPosition(enemy.getBounds().left - player.getBounds().width,
                             player.getBounds().top);
          player.getVelocity().x = -150.f;
        } else {
          player.setPosition(enemy.getBounds().left + enemy.getBounds().width,
                             player.getBounds().top);
          player.getVelocity().y = -150.f;
        }
        player.getVelocity().y = -150.f;
